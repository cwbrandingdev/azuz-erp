...
const BLOCKS: { key: CashFlowBlock; title: string }[] = [
  { key: CashFlowBlock.OPERATIONAL, title: '01 Fluxo Operacional' },
  {
    key: CashFlowBlock.FINANCIAL_MOVEMENTS,
    title: '02 Movimentações Financeiras e dos Sócios',
  },
  {
    key: CashFlowBlock.OWN_ACCOUNT_TRANSFER,
    title: '03 Transferências entre Contas Próprias',
  },
];

type TxWithCategory = Prisma.FinancialTransactionGetPayload<{
...
function effectiveDate(tx: { dueDate: Date | null; date: Date }) {
  return tx.dueDate ?? tx.date;
}

function signedCash(tx: { type: TransactionType; amount: Prisma.Decimal }) {
  const amount = Number(tx.amount);
  return tx.type === TransactionType.INCOME ? amount : -amount;
}
...
function dreBucket(tx: TxWithCategory): { bucket: DreBucket; amount: number } {
  const amount = Number(tx.amount);
  const group = tx.category?.dreGroup;

  if (group === DreGroup.TRANSFER) return { bucket: 'ignore', amount: 0 };
  if (group === DreGroup.DEDUCTION) return { bucket: 'deduction', amount };
  if (group === DreGroup.VARIABLE_COST) return { bucket: 'variable', amount };
  if (group === DreGroup.FIXED_EXPENSE) return { bucket: 'fixed', amount };
  if (group === DreGroup.PROFIT_DISTRIBUTION) {
    return { bucket: 'distribution', amount };
  }
  if (group === DreGroup.FINANCIAL_RESULT) {
    return {
      bucket: 'financial',
      amount: tx.type === TransactionType.INCOME ? amount : -amount,
    };
  }
  if (tx.type === TransactionType.INCOME) return { bucket: 'revenue', amount };
...
export class FinanceReportsService {
  ...
  async getManagementDashboard(query: QueryDateRangeDto) {
    ...
    for (const tx of paidTx) {
      ...
      if (tx.bankAccountId && balanceByBank.has(tx.bankAccountId)) {
        balanceByBank.set(
          tx.bankAccountId,
          (balanceByBank.get(tx.bankAccountId) ?? 0) + signed,
        );
      } else {
        unassigned += signed;
      }
    ...
    const overdueIncome = openTx.filter(
      (tx) =>
        tx.type === TransactionType.INCOME &&
        effectiveDate(tx).getTime() < todayStart.getTime(),
    );
    const overdueExpense = openTx.filter(
      (tx) =>
    ...
    const horizons = [7, 15, 30].map((days) => {
      ...
      for (const tx of openTx) {
        ...
        if (when < todayStart || when > horizonEnd) continue;
        if (tx.type === TransactionType.INCOME) income += Number(tx.amount);
    ...
    for (const tx of periodTx) {
      ...
      if (part.bucket === 'ignore') continue;
    ...
    const breakEven =
      marginRatio > 0 ? totals.fixed / marginRatio : totals.fixed;
    ...
    const percent = (value: number) =>
      totals.revenue > 0 ? money((value / totals.revenue) * 100) : 0;
    ...
    return {
      ...
      overdue: {
        ...
        incomeAmount: money(
          overdueIncome.reduce((sum, tx) => sum + Number(tx.amount), 0),
        ),
        expenseCount: overdueExpense.length,
        expenseAmount: money(
          overdueExpense.reduce((sum, tx) => sum + Number(tx.amount), 0),
        ),
      },
      projection: horizons,
  ...
  async getCashFlowStatement(query: QueryCashFlowStatementDto) {
    await this.financeService.ensureChartOfAccounts();
    const now = new Date();
    const from = query.from
      ? parseStart(query.from)
    ...
    const transactions = await this.prisma.financialTransaction.findMany({
      where: {
        ...
        status: query.status
          ? query.status
          : { in: [TransactionStatus.PAID, TransactionStatus.PENDING, TransactionStatus.OVERDUE] },
    ...
    const blocks = BLOCKS.map((block) => {
      const entries = transactions
        .filter((tx) => (tx.category?.cashFlowBlock ?? CashFlowBlock.OPERATIONAL) === block.key)
  ...
  async getProjectedCashFlow(query: QueryProjectedCashFlowDto) {
    await this.financeService.ensureChartOfAccounts();
    const todayEnd = endOfToday();
    ...
    const currentBalance = money(
      paid.reduce((sum, tx) => sum + signedCash(tx), 0),
    );
    ...
    const grouped = new Map<string, TxWithCategory[]>();

    for (const tx of pending) {
      const when = effectiveDate(tx);
      if (when < tomorrow) continue;
      if (limit && when > limit) continue;
  ...
  async getAnnualDre(query: QueryAnnualDreDto) {
    await this.financeService.ensureChartOfAccounts();
    ...
    const paid = transactions.filter((tx) => tx.status === TransactionStatus.PAID);
    ...
    const monthValue = (section: { totals: number[] }, month: number) =>
      section.totals[month] ?? 0;

    const managerial = Array.from({ length: 12 }, (_, month) =>
      money(
        monthValue(sections[0], month) +
    ...
    const finalResult = managerial.map((value, index) =>
      money(value + monthValue(sections[5], index)),
    );

    let running = 0;
    const availableBalance = Array.from({ length: 12 }, (_, month) => {
      for (const tx of paid) {
        if (tx.date.getMonth() !== month) continue;
  ...
  private section(
    ...
    include: (tx: TxWithCategory) => boolean,
    invertExpenseSign: boolean,
  ) {
    const rows = new Map<
      string,
      { categoryId: string; code: string | null; name: string; months: number[]; total: number }
    >();
    ...
    for (const tx of transactions) {
      ...
      const part = dreBucket(tx);
      const display =
        invertExpenseSign && part.bucket !== 'financial'
          ? part.amount
      ...
      row.months[tx.date.getMonth()] = money(
        row.months[tx.date.getMonth()] + signed,
      );
    ...
    const totals = Array.from({ length: 12 }, (_, month) =>
      money(list.reduce((sum, row) => sum + row.months[month], 0)),
    );
  ...
  private toEntry(tx: TxWithCategory) {
    return {
      id: tx.id,
